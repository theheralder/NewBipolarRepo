...
var accelSide = 0.05;
var colBoxSize = 20;
var playerOffsetX = 48; //offset from top left corner of image to center
var playerOffsetY = 38; //offset from top left corner of image to center

var glassOffsetX = -720; //offset from top left corner of image to center
var glassOffsetY = -480; //offset from top left corner of image to center

var orbOffsetX = 0; //43; //offset from top left corner of image to center
var orbOffsetY = 0; //43; //offset from top left corner of image to center
...
function init()
{
 ...
 player.y = canvas.height / 2;*/
	...
	player.offsetX = playerOffsetX;
	player.offsetY = playerOffsetY;
...
function tick()
{
	...
	if(SEEK)
	{
		for(var i = 0; i < orbs.length; i++)
		{
			...
			orbs[i].velx = ((player.x + player.offsetX) - (orbs[i].x + orbs[i].offsetX))/(Math.sqrt(distSq(orbs[i],player))) / 2;
			if(upHeld)
			{
	...
	for(var i = 0; i < glass.length; i++)
	{
		if((
			//*/
			player.x + player.offsetX) - colBoxSize < glass[i].x
			&& (player.x + player.offsetX) + colBoxSize > glass[i].x
			&& (player.y + player.offsetY) - colBoxSize < glass[i].y
			&& (player.y + player.offsetY) + colBoxSize > glass[i].y
			/*/
			player.x + player.offsetX) - colBoxSize < (glass[i].x + glass.offsetX)
			&& (player.x + player.offsetX) + colBoxSize > (glass[i].x + glass.offsetX)
			&& (player.y + player.offsetY) - colBoxSize < (glass[i].y + glass.offsetY)
			&& (player.y + player.offsetY) + colBoxSize > (glass[i].y + glass.offsetY)
			//*/
			)
		{
	...
	for(var i = 0; i < orbs.length; i++)
	{
		if((player.x + player.offsetX) - colBoxSize < (orbs[i].x + orbs[i].offsetX)
			&& (player.x + player.offsetX) + colBoxSize > (orbs[i].x + orbs[i].offsetX)
			&& (player.y + player.offsetY) - colBoxSize < (orbs[i].y + orbs[i].offsetY)
			&& (player.y + player.offsetY) + colBoxSize > (orbs[i].y + orbs[i].offsetY))
		{
			knockBack();
			orbs.splice(i, 1);
...
function createGlassWave()
{
	...
	{
		glassInst = new createjs.Bitmap("Shard 01.png");
		glassInst.setTransform(canvas.width / 2, canvas.height / 2, 0.3, 0.3);
		glassInst.regX = glassInst.image.width * 0.5;
		glassInst.regY = glassInst.image.height * 0.5;
		...
		glassInst.offsetX = glassOffsetX;
		glassInst.offsetY = glassOffsetY;
...
function createOrb()
{
	...
	orbInst.offsetX = orbOffsetX;
	orbInst.offsetY = orbOffsetY;
...
function distSq(a,b)
{
	var distance = (((b.x + b.offsetX) - (a.x + a.offsetX)) * ((b.x + b.offsetX) - (a.x + a.offsetX))) + (((b.y + b.offsetY) - (a.y + a.offsetY)) * ((b.y + b.offsetY) - (a.y + a.offsetY)));
	return distance;
}
